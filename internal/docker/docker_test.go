...
func TestStatusOf(t *testing.T) {
	noState := dockertypes.ContainerJSON{ContainerJSONBase: &dockertypes.ContainerJSONBase{}}
...
func TestContainerFromSummary(t *testing.T) {
	cases := []struct {
		name string
		in   dockertypes.Container
		want Container
	}{
		{
			name: "name trimmed of leading slash",
			in:   dockertypes.Container{ID: "abc123", Names: []string{"/grav"}, Image: "getgrav/grav:latest", State: "running", Status: "Up 3 hours"},
		...
		},
		{
			...
			in:   dockertypes.Container{ID: "def456", State: "exited"},
			want: Container{ID: "def456", Name: "def456", State: "exited"},
		},
		{
			name: "doupro.enable=false marks Excluded",
			in:   dockertypes.Container{ID: "ghi789", Names: []string{"/svc"}, Labels: map[string]string{"doupro.enable": "false"}},
		...
		},
		{
			...
			in:   dockertypes.Container{ID: "jkl012", Names: []string{"/db"}, Labels: map[string]string{"com.docker.compose.project": "business"}},
